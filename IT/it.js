...
async function fetchTasks() {
  ...
  try {
    ...
    // 🔹 Department filter (change per page)
    const department = "IT"; // Example: IT department

    // 🔹 Show only tasks assigned BY or assigned TO this department
    allTasks = data.filter((t) => {
      const assignedBy = (t["ASSIGNED BY"] || "").trim().toLowerCase();
      const assignedTo = (t["ASSIGNED TO"] || "").trim().toLowerCase();
      const dept = department.toLowerCase();
      return assignedBy === dept || assignedTo === dept;
    });
